  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-32 left-8 z-[501] w-16 h-16 bg-slate-900 text-yellow-500 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all group border border-yellow-500/20"
      >
        <Bot size={32} />
        ...
        <div className={`fixed inset-0 md:inset-auto md:bottom-24 md:left-8 md:w-[400px] md:h-[600px] z-[5000] bg-white md:rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-500 ${lang === 'ar' ? 'font-arabic' : ''}`}>
          ...
          <div className="bg-slate-900 p-6 flex items-center justify-between text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-sm uppercase tracking-widest">{t.chatbot.title}</h3>
          ...
          <div ref={scrollRef} className="flex-grow overflow-y-auto p-6 space-y-4 bg-slate-50">
            {messages.length === 0 && (
              <div className="text-center py-10 opacity-50">
                ...
                <p className="text-sm font-bold text-slate-400">{t.chatbot.welcome}</p>
              </div>
            )}
            {messages.map((m, i) => (
          ...
          <div className="p-6 bg-white border-t border-slate-100">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl pr-14 focus:border-yellow-500 outline-none transition-all font-bold text-slate-900"
              />
              <button 
                onClick={handleSend}
                disabled={!input.trim() || isTyping}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-slate-900 text-yellow-500 rounded-xl hover:bg-yellow-500 hover:text-slate-900 transition-all disabled:opacity-50"
              >
                <Send size={18} />
              </button>
            </div>
