...
CREATE TABLE visits (
  ...
  page TEXT,
  ip_address TEXT -- Colonne pour l'adresse IP
);
    ...
    <div className="fixed inset-0 z-[1000] bg-slate-950 flex flex-col overflow-hidden text-slate-200 font-sans">
      ...
      <header className="bg-slate-900/95 backdrop-blur-2xl border-b border-white/10 px-4 md:px-6 py-4 flex items-center justify-between shrink-0 shadow-2xl relative z-20">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <button 
            onClick={() => setViewMode(viewMode === 'messages' ? 'visits' : 'messages')}
            className={`flex items-center gap-2 px-3 py-2 bg-white/5 border rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === 'visits' ? 'text-gold-500 border-gold-500/50' : 'text-slate-400 border-white/10'}`}
          >
             {viewMode === 'messages' ? <Eye size={14} /> : <MessageSquare size={14} />} 
             <span className="hidden xs:inline">{viewMode === 'messages' ? 'Voir Visites' : 'Voir Messages'}</span>
          </button>
        ...
        <div className="bg-slate-900 border-b border-white/10 p-6 md:p-8 animate-in slide-in-from-top-4 duration-500 shadow-2xl relative z-30 shrink-0 max-h-[80vh] overflow-y-auto">
          <div className="max-w-5xl mx-auto">
            <div className="flex gap-4 mb-8 border-b border-white/5 pb-4">
               ...
               <button onClick={() => setActiveTab('fix')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest ${activeTab === 'fix' ? 'bg-red-500 text-white' : 'text-slate-400'}`}>SQL V11 (IP)</button>
            </div>
            {activeTab === 'config' ? (
              ...
              <div className="space-y-4">
                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
                   <p className="text-xs text-slate-300 mb-4 font-bold flex items-center gap-2"><ShieldAlert size={14} /> Script de mise à jour pour le Tracking (V11) :</p>
                   <p className="text-[10px] text-red-400 mb-2">Attention: Ce script va effacer les anciennes données pour ajouter le support IP.</p>
                   ...
                   <button onClick={() => { navigator.clipboard.writeText(sqlUniversalFixV11); alert("Copié !"); }} className="mt-4 px-4 py-2 bg-gold-500 text-slate-950 rounded-lg text-[10px] font-black uppercase">Copier Script V11</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex-grow overflow-y-auto p-4 md:p-8 bg-slate-950/50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12">
            <div className={`bg-slate-900 border p-6 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center cursor-pointer transition-all ${viewMode === 'visits' ? 'border-gold-500' : 'border-white/5'}`} onClick={() => setViewMode('visits')}>
               <Users size={20} className="text-gold-500 mb-2" />
               <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Total Visiteurs (Uniques)</p>
               <p className="text-2xl md:text-3xl font-black text-white">{visits.length}</p>
            </div>
            <div className={`bg-slate-900 border p-6 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center cursor-pointer transition-all ${viewMode === 'messages' ? 'border-blue-500' : 'border-white/5'}`} onClick={() => setViewMode('messages')}>
            ...
            <div className={`bg-slate-900 border p-6 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center transition-all duration-1000 ${lastRtEvent?.includes('VISITEUR') ? 'border-green-500/50 scale-105 bg-green-500/5' : 'border-white/5'}`}>
               ...
               <p className="text-[10px] font-black text-slate-300 uppercase truncate max-w-full">
                  {visits[0]?.ip_address || 'Aucune IP'}
               </p>
            </div>
            <div className="hidden lg:flex bg-slate-900 border border-white/5 p-6 rounded-3xl shadow-xl flex-col items-center justify-center text-center">
               ...
               <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Session Tracking</p>
               <p className="text-xs font-black text-green-400 uppercase tracking-widest">Activé</p>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between mb-8">
             <div className="relative max-w-md w-full">
               ...
               <input 
                 type="text" 
                 placeholder={viewMode === 'messages' ? "Chercher dans les messages..." : "Filtrer par IP ou Page..."}
             ...
             {viewMode === 'messages' ? (
               ...
               visits.length === 0 ? (
                 <div className="py-20 text-center opacity-20">
                    ...
                    <p className="text-lg font-black uppercase">Aucune visite enregistrée</p>
                 </div>
               ) : (
                        ...
                        {visits.filter(v => 
                          ...
                          <tr key={v.id} className="hover:bg-white/5 transition-colors group">
                            <td className="px-6 py-4 text-slate-300 font-medium">
                               ...
                               <span className="text-[9px] text-slate-500">{new Date(v.timestamp).toLocaleDateString()}</span>
                            </td>
                            <td className="px-6 py-4">
                               <div className="flex items-center gap-2 text-gold-500 font-mono font-bold">
                                  <Globe size={14} className="opacity-40" />
                                  {v.ip_address || 'N/A'}
                                </div>
                            </td>
                            <td className="px-6 py-4">
                               <span className="bg-white/5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter text-slate-400 border border-white/5">
                                  {v.page || 'Home'}
                               </span>
                            </td>
                            <td className="px-6 py-4 text-right">
                               <a 
                                 ...
                                 target="_blank" 
                                 rel="noreferrer"
                                 className="p-2 text-slate-500 hover:text-gold-500 transition-colors inline-flex items-center gap-1 text-[9px] font-black uppercase"
                               >
