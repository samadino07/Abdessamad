...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ messages, onClose, onRefresh, onLogout, onDelete, onMarkRead, onSaveConfig, onTestPropagation, currentSbConfig, rtStatus, lastRtEvent }) => {
  ...
  const sqlNuclearResetV8 = `-- SCRIPT V8 (COMPATIBILITÉ MAXIMALE)
-- 1. On nettoie tout
DROP TABLE IF EXISTS messages;

-- 2. On crée la table simple (Auto-Increment ID)
...
-- 3. Désactiver RLS (Mode Public Total)
ALTER TABLE messages DISABLE ROW LEVEL SECURITY;

-- 4. Droits Absolus
GRANT ALL ON TABLE messages TO anon, authenticated, postgres, service_role;
GRANT ALL ON SEQUENCE messages_id_seq TO anon, authenticated, postgres, service_role;
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
               <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${rtStatus === 'ACTIF' ? 'text-green-400 bg-green-500/10 border-green-500/20' : 'text-orange-400 bg-orange-500/10 border-orange-500/20'}`}>
                  ...
                  {rtStatus === 'ACTIF' ? 'LIAISON CLOUD OK' : `STATUT : ${rtStatus || 'OFFLINE'}`}
               </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border-b border-white/10 p-8 animate-in slide-in-from-top-4 duration-500 shadow-2xl relative z-30 shrink-0 max-h-[85vh] overflow-y-auto">
          <div className="max-w-5xl mx-auto">
            <div className="flex gap-4 mb-8 border-b border-white/5 pb-6">
               <button onClick={() => setActiveTab('config')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'config' ? 'bg-gold-500 text-slate-950 shadow-lg' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>Configuration</button>
               <button onClick={() => setActiveTab('fix')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'fix' ? 'bg-red-500 text-white shadow-lg' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>Réparation (SQL V8)</button>
            </div>

            {activeTab === 'config' ? (
              <div className="grid md:grid-cols-2 gap-10 animate-in fade-in duration-300">
                <div className="space-y-6">
                  <div className={`p-6 rounded-3xl border transition-all duration-500 ${lastRtEvent?.includes('ERREUR') ? 'bg-red-500/10 border-red-400/40' : 'bg-black/40 border-white/5'}`}>
                    <h3 className="flex items-center gap-2 text-gold-500 font-black uppercase text-[10px] tracking-widest mb-4"><Activity size={16} /> Moniteur Live</h3>
                    <div className="space-y-3">
                       <div className="flex justify-between items-center text-xs">
                          <span className="text-slate-500 italic">Dernier Event :</span>
                          <span className={`font-mono text-[10px] ${lastRtEvent?.includes('ERREUR') ? 'text-red-400' : 'text-green-400'}`}>
                  ...
                  <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex gap-3 items-center">
                     ...
                     <p className="text-[10px] text-slate-300 leading-relaxed">
                        Si vos messages ne partent pas du formulaire, vérifiez que l'URL et la Clé sont correctes ci-contre.
                     </p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="text-gold-500 font-black uppercase text-[10px] tracking-widest flex items-center gap-2"><HardDrive size={14} /> Identifiants Supabase</h3>
                  <form onSubmit={(e) => { e.preventDefault(); onSaveConfig(sbUrl, sbKey); }} className="space-y-3">
                    <input type="text" placeholder="URL Supabase (ex: https://abc.supabase.co)" className="w-full bg-slate-950 border border-white/10 p-4 rounded-2xl text-sm outline-none focus:border-gold-500" value={sbUrl} onChange={e => setSbUrl(e.target.value)} />
                    <input type="password" placeholder="Clé Anon / Public" className="w-full bg-slate-950 border border-white/10 p-4 rounded-2xl text-sm outline-none focus:border-gold-500" value={sbKey} onChange={e => setSbKey(e.target.value)} />
                    <button className="w-full py-4 bg-gold-500 text-slate-950 rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">Connecter le Cloud</button>
                  </form>
                </div>
              </div>
            ) : (
              <div className="animate-in fade-in duration-300 space-y-6">
                <div className="bg-red-500/10 border border-red-500/20 p-8 rounded-[40px] space-y-4">
                   <div className="flex items-center gap-4 text-red-500">
                      <ShieldAlert size={32} />
                      <h3 className="font-black uppercase tracking-tighter text-xl text-white">Script SQL V8 (Reset Complet)</h3>
                   </div>
                   <p className="text-slate-400 text-xs italic">Ce script réinitialise la table pour garantir la compatibilité totale avec le formulaire d'envoi.</p>
                   <div className="relative">
                      ...
                      <button 
                        onClick={() => { navigator.clipboard.writeText(sqlNuclearResetV8); alert("Script V8 Copié !"); }}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
             ...
             <div className="flex gap-4">
                <div className="bg-slate-900 border border-white/5 rounded-2xl px-8 py-4 text-center shadow-2xl">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Non lus</p>
          ...
          <div className="space-y-6 pb-20">
             {filteredMessages.length === 0 ? (
               <div className="py-32 text-center opacity-20 flex flex-col items-center">
                  ...
                  <p className="text-2xl font-black uppercase tracking-tighter">Aucune donnée</p>
                  <p className="text-xs font-bold mt-2">Le Cloud est vide ou déconnecté.</p>
               </div>
             ) : (
