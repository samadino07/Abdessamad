...
const ServiceDetail: React.FC<ServiceDetailProps> = ({ service, onClose, lang, t }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center overflow-hidden">
      ...
      <div 
        className={`relative w-full h-full bg-white transition-all duration-700 cubic-bezier(0.23, 1, 0.32, 1) flex flex-col overflow-y-auto ${
          isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
        }`}
      >
        {/* Header / Navigation */}
        <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between">
          <button 
            onClick={onClose}
            className={`flex items-center space-x-2 text-slate-900 font-bold hover:text-yellow-500 transition-colors ${lang === 'ar' ? 'flex-row-reverse space-x-reverse' : ''}`}
          >
          ...
          <div className="flex items-center space-x-3">
            <span className="text-2xl font-black text-slate-900">GOLDGEN</span>
          ...
          <button 
            onClick={onClose}
            className="p-3 bg-slate-100 text-slate-900 rounded-full hover:bg-yellow-500 transition-all active:scale-90"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="container mx-auto px-6 -mt-20 relative z-20 pb-24">
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-16">
              ...
              {/* Introduction Card */}
              <div className={`bg-white p-8 md:p-16 rounded-[48px] shadow-2xl border border-slate-50 ${lang === 'ar' ? 'text-right' : ''}`}>
                ...
                <h3 className="text-3xl md:text-4xl font-black text-slate-900 mb-8 leading-tight">{service.fullDesc}</h3>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  {service.details.map((detail: string, idx: number) => (
                    <div key={idx} className={`flex items-start space-x-4 p-6 bg-slate-50 rounded-3xl border border-slate-100 group hover:bg-yellow-50 transition-all ${lang === 'ar' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                      ...
                      <span className="text-slate-900 font-bold text-lg">{detail}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className={`${lang === 'ar' ? 'text-right' : ''}`}>
                ...
                <div className="grid md:grid-cols-3 gap-8 relative">
                   ...
                   {service.steps.map((step: any, idx: number) => (
                     <div key={idx} className="relative z-10 bg-white p-8 rounded-[32px] border border-slate-100 shadow-xl hover:shadow-yellow-500/10 transition-shadow">
                        <div className="w-16 h-16 bg-slate-900 text-white rounded-2xl flex items-center justify-center text-2xl font-black mb-8 shadow-2xl">
                          0{idx + 1}
                        </div>
