          ...
          <div className="relative z-10 grid lg:grid-cols-5 gap-16">
            <div className={`lg:col-span-2 space-y-10 ${lang === 'ar' ? 'text-right' : ''}`}>
              ...
              <div className="space-y-6">
                <a href={`https://wa.me/212${CONTACT_DATA.whatsapp.substring(1)}`} className={`flex items-center space-x-6 p-6 bg-white/5 border border-white/10 rounded-3xl hover:bg-yellow-500 group transition-all ${lang === 'ar' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <MessageCircle size={28} className="text-yellow-500 group-hover:text-slate-900" />
                  <div><p className="text-xs font-black text-yellow-500 group-hover:text-slate-900 uppercase mb-1">{t.whatsapp}</p><p className="text-xl font-bold text-white group-hover:text-slate-900">{CONTACT_DATA.whatsapp}</p></div>
                </a>
              </div>
            </div>
